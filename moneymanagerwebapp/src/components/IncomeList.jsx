...
const IncomeList = ({transactions, onDelete,onDownload,onEmail}) => {
    ...
    return (
        <div className="card">
                ...
                <div className="flex items-center justify-end gap-2">
                    <button disabled={loading} className="card-btn" onClick={handleEmail}>
                        {loading?(
                            ...
                            <>
                                <Mail size={15} className="text-green-500"/>Email
                            </>
                        )}
                    </button>
                    <button disabled={loading} className="card-btn" onClick={handleDownload}>
                       {loading? (
                        <>
                         ...
                         <>
                             <Download size={15} className="text-green-500"/>Download
                         </>
                       )}
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 ">
                {/* display the incomes */}
